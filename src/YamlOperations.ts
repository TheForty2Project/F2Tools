...
export class YamlTaskOperations {
  ...
  static async getYamlKeyValues(yamlKeys: string[], idLink: boolean, activeDoc: vscode.TextDocument): Promise<string[]> {
    let yamlKeyValues: string[] = [];
    const yamlDoc = await this.parseYamlDoc(activeDoc.uri);
    let parentYamlObj: any = yamlDoc.get(yamlKeys[0], true);
    let parentKeyValue = YamlTaskOperations.getParentValue(parentYamlObj, idLink ? "Id" : "", yamlKeys[0]);
