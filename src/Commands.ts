...
{
  public static async executeQuery(itemManager: IItemManager)
  {
    const queryExecution = new QueryExecution(itemManager);
  ...
  static async loadAllFilesFromWS(itemManager: IItemManager)
  {
    ...
    let item = await itemManager.tryGetItem(F2Link.CreateFromParts([], []));

    OutputChannelLogger.logInfo(`Parsing 1st round took ${(performance.now() - start).toFixed(4)} ms`);

    start = performance.now();
    item = await itemManager.tryGetItem(F2Link.CreateFromParts([], []));
